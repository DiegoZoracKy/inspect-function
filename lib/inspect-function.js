...
const splitSkip = require('../../magicli/lib/split-skip');
const unpack = require('../../magicli/lib/unpack');
...
const getParametersStringfied = fn => {
	...
	return unpack(fnString);
}

function getParametersArray(paramsString) {
	return splitSkip(paramsString, ',', (state, char, i) => {
...
function getDestructuredParameterNames(param, list = []) {
	param = param.trim();

	if (isArrayLike(param)) {
		const unpackedArray = getParametersArray(unpack(param));
		unpackedArray.forEach(param => getDestructuredParameterNames(param, list));
	}
		...
		unpackedArray.forEach(param => {
			...
			if(objectProperty){
				...
				getDestructuredParameterNames(param, list);
			} else {
				getDestructuredParameterNames(param, list);
			}
		});
